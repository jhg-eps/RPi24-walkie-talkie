...
#define enable_radio() gpio_write(enable_pin, GPIO_HIGH) // possible GPIO competition here. this controls CE on the RF24
#define disable_radio() gpio_write(enable_pin, GPIO_LOW) // possible gpio competition here.
...
uint8_t read_register(uint8_t reg) {
  ...
  uint8_t commandbyte = (R_REGISTER | (REGISTER_MASK & reg));
  char tbuf[] = {(char)commandbyte, 'a'};
  char rbuf[] = {'F', 'F', 'F', 'F', 'F', 'F', 'F', 'F', 'F', 'F'};  
  
  spi_enable(spi);
  bcm2835_spi_transfernb(tbuf, rbuf, 3);
  result = (uint8_t)rbuf[1];
  if (reg == 0x07)
	result = (uint8_t)rbuf[0];  // if we're reading STATUS, we want the first byte returned
  spi_disable(spi);
  return result;
}
...
void toggle_features() {
  spi_enable(spi);
  spi_transfer(ACTIVATE, NULL);
  spi_transfer(ACTIVATE_2, NULL);
  spi_disable(spi);
}
...
uint8_t get_dyn_payload_len() {
  uint8_t result = 0;
  spi_enable(spi);
  spi_transfer(R_RX_PL_WID, NULL);
  spi_transfer(0xff, &result);
  spi_disable(spi);
  return result;
}
...
void rf24_writeAckPayload(uint8_t pipe, const void* buf, uint8_t len) {
  ...
  spi_enable(spi);
  spi_transfer(W_ACK_PAYLOAD | (pipe & 0b111), NULL);
